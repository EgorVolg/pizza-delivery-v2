import styles from "./DeleteMenu.module.css";
import {
  useDeleteCartItemMutation,
  useClearCartItemMutation,
  useGetCartItemsQuery,
} from "../../entities/cart/model/cart.api";
import { useDispatch } from "react-redux";
import Container from "../../shared/ui/Container/Container";
import Button from "../../shared/ui/Button/Button";
import { EmptyCart } from "../../widgets/Cart/ui/EmptyCart";

export const DeleteMenu = () => {
  const dispatch = useDispatch();
  const { data: cartItems } = useGetCartItemsQuery();
  const [clearCart] = useClearCartItemMutation();
  const [deleteFromCart] = useDeleteCartItemMutation();

  const handleDeleteFromCart = (id: number) => {
    deleteFromCart(id);
  };

  const handleCloseMenu = () => {
    dispatch({ type: "deleteMenu/setCloseOpenDeleteMenu", payload: false });
  };

  const handleClearCart = () => {
    clearCart("clear");
  };

  if (!cartItems) return null;

  return (
    <div className={styles.deleteMenu}>
      {cartItems.data.length > 0 && (
        <button className={styles.close} onClick={handleCloseMenu}>
          <svg
            width="30"
            height="30"
            viewBox="0 0 30 30"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M17.4492 14.9174L29.5992 2.76743C29.8723 2.44859 30.015 2.03847 29.9988 1.61901C29.9826 1.19955 29.8087 0.801647 29.5118 0.504823C29.215 0.207998 28.8171 0.0341104 28.3977 0.0179084C27.9782 0.00170651 27.5681 0.144384 27.2492 0.417428L15.0992 12.5674L2.94924 0.400761C2.6304 0.127717 2.22028 -0.0149589 1.80082 0.00124298C1.38136 0.0174449 0.983459 0.191331 0.686635 0.488156C0.389811 0.78498 0.215923 1.18288 0.199721 1.60234C0.183519 2.0218 0.326196 2.43192 0.59924 2.75076L12.7492 14.9174L0.582573 27.0674C0.408104 27.2168 0.266403 27.4007 0.166364 27.6075C0.0663255 27.8143 0.0101078 28.0395 0.00124197 28.269C-0.00762386 28.4985 0.0310527 28.7274 0.114844 28.9413C0.198635 29.1552 0.325732 29.3494 0.488156 29.5118C0.650581 29.6743 0.844826 29.8014 1.0587 29.8852C1.27258 29.9689 1.50146 30.0076 1.731 29.9988C1.96053 29.9899 2.18575 29.9337 2.39252 29.8336C2.5993 29.7336 2.78316 29.5919 2.93257 29.4174L15.0992 17.2674L27.2492 29.4174C27.5681 29.6905 27.9782 29.8331 28.3977 29.8169C28.8171 29.8007 29.215 29.6269 29.5118 29.33C29.8087 29.0332 29.9826 28.6353 29.9988 28.2158C30.015 27.7964 29.8723 27.3863 29.5992 27.0674L17.4492 14.9174Z"
              fill="white"
            />
          </svg>
        </button>
      )}

      <Container className={styles.deleteMenu_container}>
        <header className={styles.deleteMenu_header}>
          <h1 className={styles.deleteMenu_title}>Удалить из корзины</h1>
          <Button
            className={styles.deleteMenu_clearButton}
            onClick={handleClearCart}
          >
            Oчистить корзину
          </Button>
        </header>
        <div className={styles.deleteMenu_line} />
        {cartItems.data.length > 0 ? (
          <ul className={styles.deleteMenu_list}>
            {cartItems.data.map((item) => (
              <div className={styles.deleteMenu_item} key={item.id}>
                <button
                  className={styles.deleteMenu_deleteButton}
                  onClick={() => handleDeleteFromCart(item.id)}
                  key={item.id}
                >
                  <svg
                    width="30"
                    height="30"
                    viewBox="0 0 30 30"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M17.4492 14.9174L29.5992 2.76743C29.8723 2.44859 30.015 2.03847 29.9988 1.61901C29.9826 1.19955 29.8087 0.801647 29.5118 0.504823C29.215 0.207998 28.8171 0.0341104 28.3977 0.0179084C27.9782 0.00170651 27.5681 0.144384 27.2492 0.417428L15.0992 12.5674L2.94924 0.400761C2.6304 0.127717 2.22028 -0.0149589 1.80082 0.00124298C1.38136 0.0174449 0.983459 0.191331 0.686635 0.488156C0.389811 0.78498 0.215923 1.18288 0.199721 1.60234C0.183519 2.0218 0.326196 2.43192 0.59924 2.75076L12.7492 14.9174L0.582573 27.0674C0.408104 27.2168 0.266403 27.4007 0.166364 27.6075C0.0663255 27.8143 0.0101078 28.0395 0.00124197 28.269C-0.00762386 28.4985 0.0310527 28.7274 0.114844 28.9413C0.198635 29.1552 0.325732 29.3494 0.488156 29.5118C0.650581 29.6743 0.844826 29.8014 1.0587 29.8852C1.27258 29.9689 1.50146 30.0076 1.731 29.9988C1.96053 29.9899 2.18575 29.9337 2.39252 29.8336C2.5993 29.7336 2.78316 29.5919 2.93257 29.4174L15.0992 17.2674L27.2492 29.4174C27.5681 29.6905 27.9782 29.8331 28.3977 29.8169C28.8171 29.8007 29.215 29.6269 29.5118 29.33C29.8087 29.0332 29.9826 28.6353 29.9988 28.2158C30.015 27.7964 29.8723 27.3863 29.5992 27.0674L17.4492 14.9174Z"
                      fill="white"
                    />
                  </svg>
                </button>
                <img
                  className={styles.deleteMenu_img}
                  src={item.imageUrl}
                  alt={item.name}
                />
                <h1 className={styles.deleteMenu_name}>{item.name}</h1>
                <div className={styles.deleteMenu_description}>
                  {item.size} см, {item.type} <br />
                  {item.toppings && `+ ${item.toppings}`}
                </div>

                <span className={styles.deleteMenu_price}>{item.price} ₽</span>
              </div>
            ))}
          </ul>
        ) : (
          <EmptyCart handleCloseCart={handleCloseMenu} />
        )}
      </Container>
    </div>
  );
};
